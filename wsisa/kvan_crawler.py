...
class KVStore:
            ...
            if row:
                ...
                cur.execute(
                    ...
                    INSERT INTO transactions (
                      ...
                      card_type, resident_front, status, message,
                    ...
                    """,
                    (new_tx_id, agency_id, amount, customer_name or "", message, approval_no, registered_at),
                )
        conn.commit()
        conn.close()
    def sync_kvan_to_transactions(self) -> bool:
        ...
        inserted = 0
        try:
            agency_mid_map = self.load_agency_mid_map()
            ...
            if self.use_json:
                ...
                for kr in krows:
                    ...
                    if not amt or not approval:
                        continue
                    agency_id = agency_mid_map.get(mid) if mid else None
                    tx_status = "success" if "승인" in tx_type else "fail" if ("취소" in tx_type or "실패" in tx_type or "오류" in tx_type) else "other"
                    ...
                    if not found:
                        for tx in tx_rows:
                            ...
                            same_agency = (not agency_id) or ((tx.get("agency_id") or "") == agency_id)
                ...
                with conn.cursor() as cur:
                    for kr in krows:
                        ...
                        cur.execute(
                            ...
                            """,
                            (new_tx_id, agency_id, amt, tx_status, message, mid, approval, tx_type, reg),
                        )
                        inserted += 1
                conn.commit()
                conn.close()
            if updated or inserted:
                print(f"[INFO] K-VAN → transactions 동기화 완료 (updated={updated}, inserted={inserted}, json={self.use_json})")
...
CONFIRM_DELETE_XPATH = (
    "//div[@role='alertdialog']//button[normalize-space()='삭제']"
    " | //div[@data-slot='alert-dialog-content']//button[normalize-space()='삭제']"
...
def _click_trash_and_confirm(driver: webdriver.Chrome, card) -> bool:
    try:
        ...
        try:
            ...
            print("[WARN] 휴지통 클릭 후 삭제 확인 버튼을 찾지 못했습니다.")
            return False
        if not _fast_click(driver, confirm_btn):
            return False
        _brief_sleep(0.08)
        return True
    except Exception as e:
