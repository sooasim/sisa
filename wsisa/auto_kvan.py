...
def _scrape_transactions_and_store(driver: webdriver.Chrome) -> None:
    ...
    try:
        ...
        snapshot_rows, body_rows, used_label, attempts, h_tr1 = (
        ...
        if not snapshot_rows and body_rows:
            print(
                ...
                f"헤더시도={used_label or '(모두실패)'} tr1={h_tr1[:8]!s} — kvan_transactions DB 유지"
            )
            try:
        ...
        conn.close()
        print(
            f"[INFO] /transactions 에서 {inserted}건 저장 (header={used_label}) kvan_transactions"
        )
    except Exception as e:
